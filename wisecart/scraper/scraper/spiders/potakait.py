...
class PotakaitSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        search_url = f"https://potakait.com/search?q={search_term}"
    ...
    def parse(self, response):
        ...
        for product in products:
            ...
            title_element = product.css("h4.title a::text").get()
            if not title_element or not self.title_contains_search_words(title_element.strip()):
                continue

            raw_price = product.css("span.regular::text").get()
            ...
            item = SearchResultItem(
                ...
                price=cleaned_price,
                url=response.urljoin(product.css("h4.title a::attr(href)").get()),
                store_id=store.id,
